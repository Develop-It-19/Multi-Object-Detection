...
import json, pdb	#pdb is python debugger
from PIL import ImageDraw, ImageFont
from matplotlib import patches, patheffects		#patches is used to draw bounding boxes    #patheffects is used to draw outline for our bounding boxes.
torch.cuda.set_device(0)	#0 means 1 GPU
...
def get_trn_anno():
	...
	for o in trn_j[ANNOTATIONS]:
		if not o['ignore']:
			bb = o[BBOX]
			bb = np.array([bb[1], bb[0], bb[3]+bb[1]-1, bb[2]+b[0]-1])
...
mc = [set([cats[p[1]] for p in trn_anno[o]]) for o in trn_ids]	#we use set so that we make sure each object is apperaing once. As we use Set for unique things.
...
mcs = np.array([np.aray([cat2id[p] for p in o]) for o in mc])
mcs
...
#torch ground truth function simply converts tensors into numpy arrays.
...
#Anchor Boxes is the same as Prior Boxes which is the same as Default Boxes.

#Jaccard Index is the same as IoU (Intersection over Union)

grid_sizes

anchors
#Coordinates of anchor Boxes (Center, Height, Width)
...
overlaps = jaccard(bbox.data, anchor_cnr.data)  #Gives the overlap between anchor boxes with each of the ground truths.
overlaps

overlaps.max(1)		#Maximum overlap of each ground truth object.
...
anc_sizes = np.concatenate([np.array([[o/ag, p/ag] for i in range(ag * ag) for o, p in anchor_scales])
			for ag in anc_grids])
grid_sizes = V(np.concatenate([np.array([1/ag for i in range(ag*ag) for o, p in anchor_sales])
